...
describe('scraper js', () => {
    ...
    it('Links exists', async () => {
        ...
        const headings = getLinks($('a'), 'http://example.com/contact');
        
        expect(headings).toEqual({'externalLinks': [{'name': 'http://twitter.com/cbracco', 'value': 2}, {'name': 'http://github.com/cbracco/html5-test-page', 'value': 1}], 'internalLinks': [{'name': 'http://example.com/about', 'value': 1}, {'name': 'http://example.com/search?term=fsdfsd', 'value': 1}, {'name': 'http://example.com', 'value': 2}]});
    });

    it('No Links', async () => {
        const $ = cheerio.load(mockNoLinksHtml);
        const headings = getLinks($('a'), 'http://example.com');
